...
* `<FlashcardEngine>`: Powers Production sessions. No fixed session length and no weighted selection — instead it maintains a rotating **working set** bounded by `[minWorking, maxWorking]`:
  ...
  * **Progress UI:** `{masteredIds.size} / {vocabList.length} mastered` counter pinned to the right of the top bar, with "Exit Session" on the left.
...
* **Dual Range Slider (`DualRangeSlider`):** Implemented as two overlapping native `<input type="range">` elements stacked over one shared painted track (the track is transparent on the inputs themselves). Each thumb uses a Material-Design-style touch target: the real hit-box is a large 44px circle (`THUMB_HIT`), but it's painted almost entirely transparent via a radial-gradient background so only a small 6px dot (`THUMB_DOT`) is visible at rest. A translucent 20px halo (`THUMB_HALO`) fades in only while a thumb has keyboard/pointer focus, via the `:focus::-webkit-slider-thumb` / `:focus::-moz-range-thumb` compound selectors — this keeps the resting UI compact while still giving an obvious, larger tap target and focus affordance. A z-index heuristic (whichever thumb's value is on the far side of the range's midpoint gets a higher z-index) keeps the correct thumb grabbable when the two cross. When both values are exactly equal, each thumb is nudged a few px apart (`THUMB_GAP`, via inline `transform: translateX(...)`) purely so they render as two distinguishable dots instead of one — the underlying values are unaffected.
