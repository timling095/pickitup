...
* **Design System (Material Design 2):** The whole UI follows the classic Android "Indigo + Pink A200" MD2 default palette, registered as Tailwind v4 theme tokens in `src/index.css` (`@theme { --color-md-primary: #3F51B5; --color-md-primary-dark: #303F9F; --color-md-primary-light: #C5CAE9; --color-md-accent: #FF4081; --color-md-accent-light: #FCE4EC; }`), which generates `bg-md-primary`, `text-md-primary`, `border-md-primary`, `bg-md-primary/12` (opacity variants), etc. across every file. `colorPrimary` (indigo) drives contained buttons, checkboxes, the slider, and selected tab text; `colorAccent` (pink) is reserved for the Terms Viewer's marked-row highlight (`bg-md-accent-light`) rather than being spread across primary controls, keeping the two-color contrast legible. Every boxed button and tab (mode toggle, Terms Viewer's All/Practicing, View Terms, Start/Resume/Discard Session, and every drill-screen button) shares one height recipe — `py-2` + `text-sm` + `uppercase tracking-wide` — matching the mode-toggle tab exactly by construction rather than by a hardcoded pixel value; chips stay visually distinct at `h-8` (32px) with natural-case labels, matching MD2's own chip-vs-button size and casing distinction. Component conventions this app follows deliberately, since they're easy to get wrong by analogy with other design systems:
  * **Outlined buttons don't get a colored border.** Per the MD2 spec, an outlined button's border is neutral and thin (`border border-slate-300` here, standing in for `onSurface` at 12% opacity) — only its *text* takes the primary color, and hover adds just a hint of it back (`hover:border-md-primary/40 hover:bg-md-primary/5`) rather than a full-strength swap. Primary color belongs on fills (contained buttons) or text (outlined/text buttons), never as a resting border tint. "View Terms" is the app's one outlined button.
  * **Filter chips don't invert text on selection, and don't need to be accented at all.** `<Chip>` in `App.tsx` uses a purely neutral grayscale treatment — `bg-white`/`text-slate-600` unselected, `bg-slate-200`/`text-slate-800` selected (the same gray as the tab-track backing behind the mode toggle and Terms Viewer tabs, so the whole app reads as one gray/indigo system) — plus a leading checkmark to signal the selected state. Chips never go through `md-primary`; MD2 doesn't require chips to carry the app's accent color, and inverting to white-on-indigo text is a contained-button pattern, not a chip one.
  * `<MdCheckbox checked onChange>`: an 18px square, 2px-border checkbox — indigo fill + a white `lucide-react` `Check` icon when checked, a plain white box with a gray border (indigo on hover) when not. The `<button>` itself *is* the full 40px halo/hit-area (not just the 18px visual box) — the 18px box is a plain inner `pointer-events-none` `<div>`, since a `<button>` can't nest another `<button>` — so a click anywhere in the halo registers, not just on the small visual square. The halo reuses `DualRangeSlider`'s thumb technique (a Material "state layer"): `group-hover`/`group-focus`/`group-active` fade it in (`group-focus`, not `-within`, since the button itself — not a descendant — receives focus here). Used for the boolean settings toggles.
  * `<Chip selected onClick>`: fully rounded (`rounded-full`), `h-8` (32px, one size step below the `py-2` button/tab height), used for every multi-select filter (Lessons, Word Type), laid out with `flex flex-wrap` (chips wrap and auto-size to their label, unlike a fixed grid) so both filter groups read as one consistent chip system.
