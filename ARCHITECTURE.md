...
* **Filters Card:** Lesson selection and word-type selection live together in one card (no card title), using the identical toggle-button mechanism (a grid of independent, multi-select pill buttons — `bg-slate-800` selected / `bg-slate-100` unselected) for both. "Lessons" and "Word Type" are rendered as `card-title`-style section labels (`text-xs font-semibold text-slate-400 uppercase tracking-wider`), consistent with the other card headers. There is no session-in-progress notice — the Filters card stays fully live and editable at all times, including while a Flashcard session is in progress — see Session Scope Snapshot below for how that's made safe.
...
* **Session Status Card:** Whenever a Flashcard session is in progress (`fcActive`), the right-column Settings card is entirely replaced by a Session Status card — a live `{sessionMasteredCount} / {sessionVocab.length} mastered` readout and a "Discard Session" button (`setFcActive(false)`) as the only way to end a session without finishing it. This applies regardless of which mode tab is currently displayed, since `fcActive` is a single global flag independent of `activeMode`.
