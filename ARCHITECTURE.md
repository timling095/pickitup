...
* **Design System:** An earlier pass reskinned the whole app in a Material Design 2 "Indigo + Pink A200" palette (an app-wide `colorPrimary` on every button/tab/slider, `uppercase` button labels, MD button-height conventions, etc.), but it didn't land well as a whole and was rolled back — buttons, tabs, and the slider are back to the original near-black (`bg-slate-800`/`text-slate-800`) styling, normal case, original sizing (`h-11`/`py-4`/`py-3` depending on the button). Two pieces of that experiment stuck around because they read well on their own, decoupled from the app-wide primary-color idea:
  * `<Chip selected onClick>` in `App.tsx`: a fully-rounded (`rounded-full`), `h-8` (32px) filter chip — `bg-white`/`text-slate-600` unselected, `bg-slate-200`/`text-slate-800` selected (the same gray as the tab-track backing elsewhere in the app) plus a leading checkmark. Purely grayscale, no accent color. Used for every multi-select filter (Lessons, Word Type), laid out with `flex flex-wrap` (chips wrap and auto-size to their label, unlike a fixed grid).
  * `<MdCheckbox checked onChange>` in `App.tsx`: an 18px square, 2px-border checkbox — black fill + a white `lucide-react` `Check` icon when checked, a plain white box with a gray border (darkens on hover) when not. The `<button>` itself *is* the full 40px halo/hit-area (not just the 18px visual box) — the 18px box is a plain inner `pointer-events-none` `<div>`, since a `<button>` can't nest another `<button>` — so a click anywhere in the halo registers, not just on the small visual square. The halo reuses `DualRangeSlider`'s thumb technique (a Material "state layer", now recolored to `bg-slate-800/[0.12]` to match): `group-hover`/`group-focus`/`group-active` fade it in (`group-focus`, not `-within`, since the button itself — not a descendant — receives focus here). Used for the boolean settings toggles.
  * `src/index.css`'s `@theme` block now only registers `--color-md-accent-light: #FCE4EC` (Material Pink 50), which is unrelated to either of the above — it's the Terms Viewer's marked-row highlight, predates the MD2 pass, and was never part of the primary-color rollback.
